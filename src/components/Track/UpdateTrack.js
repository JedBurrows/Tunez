...
const UpdateTrack = ({ classes, track }) => {
  ...
  const handleAudioUpload = async () => {
    try {
      ...
      data.append('cloud_name', 'jedb97');
      const res = await axios.post('https://api.cloudinary.com/v1_1/jedb97/raw/upload', data);
      return res.data.url;
    }
    catch (err) {
  ...
  const handleSubmit = async (event, updateTrack) => {
    ...
    setSubmitting(true);
    // upload audio files, get returned url
          ...
          return (
            ...
            <Dialog open={open} className={classes.dialog}>
              <form onSubmit={event => handleSubmit(event, updateTrack)}>
                <DialogTitle>UpdateTrack</DialogTitle>
                <DialogContent>
                  <DialogContentText>
                ...
                <DialogActions>
                  ...
                  <Button
                    disabled={
                      !title.trim() || !description.trim() || !file || submitting
                    }
                    type="submit"
